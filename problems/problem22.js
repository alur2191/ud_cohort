...
function findWordsWithPrefix(prefix, suffix, length){
	const wordsWithPrefix = [];
	for(let i = 0; i < words.length; i++){
		words[i].startsWith(prefix) && words[i].endsWith(suffix) && length === words[i].length && wordsWithPrefix.push(words[i])
	}
	return wordsWithPrefix;
};

console.log(findWordsWithPrefix("UN", "LY", 15));